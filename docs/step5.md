...
for文にはたくさんのデータの列のような繰り返し反復して処理できる仕組みをinの後に指定する必要があります  
ここではrange()を使ってみます  
...
range(回数)というものはrange(開始値, 停止値, 間隔)という記述を簡略化したものです  
...
while文は条件がTrueである限りブロック内を何回でも繰り返すことが可能です  
